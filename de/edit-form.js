...
function populateFormWithData(formData) {
    ...
    try {
        ...
        // Определяем "За кого"
        const isFamily = formData.for_who === 'Familie/Freundeskreis';
...
function initEditForm() {
    ...
    if (!editForm) {
        ...
        showError('Formular wurde auf der Seite nicht gefunden');
        return;
    }
    ...
    if (!code) {
        showError('Ungültiger oder abgelaufener Bearbeitungslink');
        return;
    }
    ...
    if (!formData) {
        showError('Ihre Daten konnten nicht geladen werden. Möglicherweise wurden sie gelöscht oder der Link ist abgelaufen.');
        return;
    }
    ...
    if (!success) {
        showError('Fehler beim Laden der Formulardaten');
        return;
    }
...
function setupEditFormSubmitHandler() {
    ...
    function updateSaveButtonState() {
        ...
        if (saveButton.disabled) {
            ...
            saveButton.textContent = 'Keine Änderungen';
        } else {
...
// 2. ОБНОВЛЯЕМ начальное состояние для отслеживания изменений
                ...
                throw new Error('Fehler beim Senden der Änderungen');
            }
